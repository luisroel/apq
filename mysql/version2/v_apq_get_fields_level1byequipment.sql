drop view if exists `v_apq_get_fields_level1byequipment`;
create view `v_apq_get_fields_level1byequipment`
as
	select
		  `ct`.*
	...
	from
		`v_apq_get_countsbyequipment` `ct`
		inner join `v_apq_get_availabilitybyequipment` `av` on
				`av`.`idruntime` 	= `ct`.`idruntime`
			and `av`.`area` 	 	= `ct`.`area`
			and `av`.`line` 	 	= `ct`.`line`
			and `av`.`idequipment` 	= `ct`.`idequipment`;

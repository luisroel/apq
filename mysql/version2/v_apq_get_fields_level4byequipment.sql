drop view if exists `v_apq_get_fields_level4byequipment`;
create view `v_apq_get_fields_level4byequipment`
as
	select
		  `rt`.`idruntime`
		, `rt`.`to`															as `time`
	...
	from
		`apq_runtime` `rt`
		inner join `v_apq_get_fields_level3byequipment` `vl` on

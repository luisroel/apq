...
IF EXISTS (SELECT * FROM SYS.OBJECTS WHERE object_id = OBJECT_ID(N'v_APQ_Get_IndexByLine') AND [TYPE] in (N'V'))
    DROP VIEW [dbo].[v_APQ_Get_IndexByLine]
GO
CREATE  VIEW [dbo].[v_APQ_Get_IndexByLine]
AS
	SELECT
		  [RT].[IdRuntime]
		, [RT].[To]																									AS [Time]
...
--		/ ([AV].[PlannedProductionTime] - [CT].[StdSetupTime] - [CT].[StdStartTime] - [AV].[StopTime])				AS [IP_Porc]
	...
	FROM
		[dbo].[APQ_Runtime] [RT]
		INNER JOIN [dbo].[v_APQ_Get_AvailabilityByLine] [AV] ON
			[AV].[IdRuntime] = [RT].[IdRuntime]
		INNER JOIN [dbo].[v_APQ_Get_CountsByLine] [CT] ON

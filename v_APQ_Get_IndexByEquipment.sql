...
IF EXISTS (SELECT * FROM SYS.OBJECTS WHERE object_id = OBJECT_ID(N'v_APQ_Get_IndexByEquipment') AND [TYPE] in (N'V'))
    DROP VIEW [dbo].[v_APQ_Get_IndexByEquipment]
GO
CREATE  VIEW [dbo].[v_APQ_Get_IndexByEquipment]
AS
	SELECT
		  [RT].[IdRuntime]
		, [RT].[To]									AS [Time]
...
--		/ ([AV].[PlannedProductionTime] - [CT].[StdSetupTime] - [CT].[StdStartTime] - [AV].[StopTime])				AS [IP_Porc]
	...
	FROM
		[dbo].[APQ_Runtime] [RT]
		INNER JOIN [dbo].[v_APQ_Get_AvailabilityByEquipment] [AV] ON
			[AV].[IdRuntime] = [RT].[IdRuntime]
		INNER JOIN [dbo].[v_APQ_Get_CountsByEquipment] [CT] ON
				[CT].[IdRuntime]	= [AV].[IdRuntime]
			AND [CT].[Area]			= [AV].[Area]
			AND [CT].[Line]			= [AV].[Line]
			AND [CT].[IdEquipment]	= [AV].[IdEquipment]
GO
